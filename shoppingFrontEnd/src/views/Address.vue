  ...
  <div class="container">
    <div class="checkout-addr">
      ...
      <div class="addr-list-wrap">
        <div class="addr-list">
          <ul >
            <li v-for="(item,index) in filterAddressList" :class="{check:item.addressId===orderAddressId}" @click="orderAddressId=item.addressId">
              <dl >
                <dt>{{item.userName}}</dt>
              ...
              <div class="addr-opration addr-del">
                <a href="javascript:;" class="addr-del-btn"  @click='popDelAddressComfirm(item.addressId)'>
        ...
        <div class="shipping-addr-more">
          <a class="addr-more-btn up-down-btn" href="javascript:;" @click="expand" :class="{open:this.initCount!==this.limit}">
            more
            <i class="i-up-down">
...
<modal :mdShow="mdShow" @close="closeDeleteConfirm">
    <template v-slot:title>
        <p>你确定要删除当前地址信息么</p>
    </template>
    <template v-slot:btnGroup>
        <a href="javascript:;" class="btn btn--m" @click="delAddress">确定</a>
        <a href="javascript:;" class="btn btn--m" @click="closeDeleteConfirm">取消</a>
    </template>
</modal>
...
export default {
  ...
  methods:{
          ...
          axios.get('/users/addressList').then(res=>{
              if(res.data.status===1){
                  ...
                  if(this.addressList.length>0){
                    let item=this.addressList.find(elem=>elem.isDefault)
                    if(item) this.orderAddressId=item.addressId;
                    else this.orderAddressId=this.addressList[0].addressId;
                  }
              }
          })
      },
      expand(){
          if(this.initCount===this.limit){
              this.limit=this.addressList.length;
          }else{
              this.limit=this.initCount;
          }
      },
      //直接传入address和this.address无关
      setDefault(addressId){
          axios.put('/users/address/setDefault',{addressId:addressId}).then(res=>{
              if(res.data.status===1){
                  this.init();
              }
          })
      ...
      popDelAddressComfirm(addressId){
          this.delAddressId=addressId;
          this.mdShow=true;
      },
      delAddress(){
          axios.delete('/users/address/del',{data:{addressId:this.delAddressId}}).then(res=>{
              if(res.data.status===1){
                  this.mdShow=false;
                  this.init();
              }
