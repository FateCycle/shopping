<template>
 <div>
    <nav-header @reload="reload"></nav-header>
    <nav-bread>
      <span>My Cart</span>
    </nav-bread>
      ...
      <div class="cart">
        ...
        <div class="item-list-wrap">
          <div class="cart-item">
            ...
            <ul class="cart-item-list" v-for="item in cartList">
              <li>
                ...
                <div class="cart-tab-5">
                  <div class="cart-item-opration">
                    <a href="javascript:;" class="item-edit-btn" @click="delCartConfrim(item)">
          ...
          <div class="cart-foot-inner">
            <div class="cart-foot-l">
              <div class="item-all-check">
                <a href="javascipt:;"  @click="toggleAllCheckEvent">
                  <span class="checkbox-btn item-check-btn" :class="{checked:toggleAllCheck}">
            ...
            <div class="cart-foot-r">
              ...
              <div class="btn-wrap">
                <a class="btn btn--red" :class="{'btn--dis':checkNum===0}" @click='checkout'>Checkout</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  computed:{
    checkNum(){
      return this.cartList.reduce((accumulator,currentValue)=>currentValue.checked?accumulator+1:accumulator,0);
    },
    toggleAllCheck(){
      return this.cartList.every(elem=>elem.checked);
    },
    sumPrice(){
      return this.cartList.reduce((accumulator,currentValue)=>{
        if(currentValue.checked)
          return accumulator+parseInt(currentValue.productNum)*parseFloat(currentValue.salePrice);
        else
          return accumulator;
      },0)
    }
  ...
  methods:{
      ...
      delCart(){
          axios.delete('/users/cart/del',{data:{productId:this.delProductItem.productId}}).then(res=>{
              if(res.data.status===1){
                  this.modalComfirm=false;
                  this.$store.commit('updateCartCount',-this.delProductItem.productNum);
                  this.init();
              }
          })
      },
      editCart(flag,item){
        ...
        axios.put('/users/cart/edit',item).then(res=>{
          if(res.data.status===1){
            this.init();
          }
        })
      ...
      toggleAllCheckEvent(){
        let curToggleAllCheck=!this.toggleAllCheck;
        //先修改在保存来得快，等服务器保存完再来渲染会是的响应变慢，降低用户体验
        this.cartList.forEach(elem=>elem.checked=curToggleAllCheck);
        axios.put('users/cart/editAll',{curToggleAllCheck:curToggleAllCheck}).then(res=>{
          if(res.data.status===1) {
            console.log('success');
          }
        })
